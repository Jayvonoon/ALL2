...
const PlaceDetails = ({place, selected, refProp, type}) => 
{
    ...
    const handleGoToStore = async () => {
        try 
        {
            ...
            console.log('Place name:', place.name);
              ...
              //addToCart({name: place.name}); //<<Causing issue redirecting
              navigate('/store');
              
            //   if (type ==='attractions')
            //   {
            //     addToCart({name:place.name});
            //     navigate('/store');
            //   }
    ...
    return(
        <Card elevation={6}>
            ...
            {/* Render the selected place name wherever you need it, for example, in the Store component */}
            {/* {selectedPlaceName && (
                <div>
                    Attraction: {selectedPlaceName}
                </div>
            )} */}
