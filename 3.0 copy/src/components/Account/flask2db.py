...
app = Flask(__name__)
app.secret_key = 'eBCKcZLVa-KwzaBN4LcKloIhoWw7xcgy8WIOBPcXOCI'
...
def get_db():
    if 'db' not in g:
        g.db = sqlite3.connect(path_db_file) # Update this with your actual database path
...
def update_password(email, new_password):
    ...
    try:
        ...
        # Fetch the existing record_id for the user
...
def checkout():
    try:
        ...
        send_confirmation_email(user_email, transactions)
...
def send_confirmation_email(email, transactions):

        smtp_server = 'smtp.gmail.com'
        smtp_port = 587  # Use the appropriate port
        sender_email = 'touristkiosk@gmail.com'  # Your email address
        sender_password = 'xlic dciu rxgd kbbk'  # Your email password
        ...
        try:
            ...
            for transaction in transactions:
                ...
                # if items:
                #     items_string = ', '.join([f"{item['item_name']}: {item['quantity']}" for item in items])
                # else:
                #     items_string = "No items in the order"
                ...
                body =  f"""<html>
                            <body>
                                ...
                                <p>Scan the QR code to gain access to services provided by {place_name} </p>
...
if __name__ == '__main__':
    app.config['JWT_SECRET_KEY'] = 'eBCKcZLVa-KwzaBN4LcKloIhoWw7xcgy8WIOBPcXOCI'  # Change this to a secure secret key
    jwt = JWTManager(app)
    app.run(port=4000)
